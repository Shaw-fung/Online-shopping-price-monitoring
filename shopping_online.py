...
url = ''
expected_price = 1.00
sj_server_addr = 'https://sctapi.ftqq.com/{}.send?title={}&desp={}'  # Server酱API接口地址
price_info_url = 'https://item.m.jd.com/item/mview2?datatype=1&cgi_source=mitem&sku=%s'  # 获取商品信息网址
...
def visit_web_get(web_addr): # 采取get获得源代码
...
def send_message(sendkey, title, desp):  # 走server酱发送信息
  return visit_web_get(sj_server_addr.format(sendkey, title, desp))
...
def env(url):
  if sj_sendkey.strip() == '':
    print('您未设置用于通知的Server酱key，将无法收到通知信息，程序退出！')
    exit()
  while url.strip() == '':
    url = input('请输入购物网址：')
  else:
